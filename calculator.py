...
def calculate(user):
    try:
        temp = TikTokApi.get_instance().search_for_users(search_term=user, count=1)
        userid = temp[0].get("user").get("uniqueId")
        secuid = temp[0].get("user").get("secUid")
        result = TikTokApi.get_instance().user_page(userID=userid, secUID=secuid, page_size=35, cursor=0).get(
            "itemList")
